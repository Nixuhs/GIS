namespace namespace1 {
  ...
  const interpret_display: HTMLInputElement = <HTMLInputElement>document.getElementById("interpret");
  const preis_display: HTMLInputElement = <HTMLInputElement>document.getElementById("preis");
  const datum_display: HTMLInputElement = <HTMLInputElement>document.getElementById("datum");
  const tasks: HTMLElement = <HTMLElement>document.querySelector("#tasks");   
  const add_button: HTMLButtonElement = <HTMLButtonElement>document.querySelector("#add-button");
  ...
  add_button.addEventListener("click", createTask);
    ...
    function retrieve() {
      if (localStorage.getItem("Folder1") === null) alert("Nothing in Folder1");
      else {
        document.getElementById("tasks").innerHTML = localStorage.getItem(
          "Folder1"
        );
